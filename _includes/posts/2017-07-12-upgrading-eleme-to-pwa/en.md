> Read at medium.com: [Upgrading Ele.me to Progressive Web Apps](//medium.com/elemefe/upgrading-ele-me-to-progressive-web-app-2a446832e509)
...
**In [Lighthouse](//developers.google.com/web/tools/lighthouse/) simulation (3G & 5x Slower CPU), we made Time-To-Interactive around 2 seconds,** and this was benchmarked on our HTTP1 test server. 
...
<iframe width="560" height="315" src="//www.youtube.com/embed/mbi_WnunJa8" frameborder="0" allowfullscreen></iframe>
...
@owencm have written a great post ["Reactive Web Design: The secret to building web apps that feel amazing"](//medium.com/@owencm/reactive-web-design-the-secret-to-building-web-apps-that-feel-amazing-b5cbfe9b7c50) covering both "Instant loads with skeleton screens" and "Stable loads via predefined sizes on elements" to improve perceived performance and user experience. Yes, we actually used both. 

What about we showing the end result after these optimizations first before entering technical nitty gritty? There you go!

<iframe width="560" height="315" src="//www.youtube.com/embed/K5JBGnMYO1s" frameborder="0" allowfullscreen></iframe>
...
<iframe width="560" height="315" src="//www.youtube.com/embed/w1ZbNsHmRjs" frameborder="0" allowfullscreen></iframe>
...
We don't want to manually craft each Skeleton Screen for each routes. It's a tedious job and we have to manually sync every change between Skeleton Screens and the actual UI components (Yes we treat every route as just a Vue component). But think about it, [Skeleton Screen is just a blank version of a page into which information is gradually loaded](//www.lukew.com/ff/entry.asp?1797). What if we bake the Skeleton Screen into the actual UI component as just a loading state so we can render Skeleton Screen out directly from it without the issue of syncing?

Thanks to the versatility of Vue, we can actually realize it with [Vue.js Server-Side Rendering](//ssr.vuejs.org/en/). Instead of using it on a real server, we use it at build time to render Vue components to strings and injected them into HTML templates.



### Fast Skeleton Painting...

Having markups in `*.html` doesn't mean that they will be painted fast, you have to make sure the [Critical Rendering Path](//developers.google.com/web/fundamentals/performance/critical-rendering-path/) is optimized for that. Many developers believed that putting script tags in the end of the body is sufficient for getting content painted before executing scripts. This might be true for browsers supporting rendering an incomplete DOM tree (e.g. streaming render), But browsers might not do that in mobile concerning slower hardwares, battery, and heats. **And even we are told that script tags with `async` or `defer` is not parser-blocking, it also doesn't mean we can get content painted before executing scripts in reality.**

![](//html.spec.whatwg.org/images/asyncdefer.svg)

First I want to clarify it a little bit. According to the [Scripting section of HTML](//www.w3.org/TR/html51/semantics-scripting.html#elementdef-script) (WHATWG living standard, the W3C's same here), `async` scripts would be evaluated as soon as it is available thus could potentially blocking parsing. Only `defer` (and not inlined) is specified to be never block parsing. That's why [Steve Souders](http://stevesouders.com/) ever posted ["Prefer DEFER Over ASYNC"](//calendar.perfplanet.com/2016/prefer-defer-over-async/). (`defer` has its own issue and we will cover it later.)
...
I think you may curious about how this change performs in the wild, so I have refined MMPWA by rendering 5000 list items rather 1000 to make the differences more obvious, and by designing it in an A/B testing manner. The code is on [Github](//github.com/Huxpro/mmpwa) and the demo is live on [huangxuan.me/mmpwa/](//huangxuan.me/mmpwa). Here is also a video for loungers.


<iframe width="560" height="315" src="//www.youtube.com/embed/3Ws7XBHrPD8" frameborder="0" allowfullscreen></iframe>
...
But wait, why is there still a bunch of guiltily parser-blocking scripts? Are them all `async`? OK, ok. For historical reasons, we do keep some parser-blocking scripts, like [lib-flexible](//github.com/amfe/lib-flexible), we couldn't get rid of it without a huge refactoring. But most of these blocking scripts are in fact `defer`. We expected that they can be executed after parsing and in order, however the profile kinda slap on my face. :(

Remember I said I would talk about one issue of `defer` previously? Yes, that's it. I have had a [conversation](//twitter.com/Huxpro/status/859842124849827841) with [Jake Archibald](//twitter.com/jaffathecake) and it turns out it might be a bug of Chrome when the deferred scripts are fully cached. [Vote it at crbug](//bugs.chromium.org/p/chromium/issues/detail?id=717979)!
...
[Alex Russell](//medium.com/@slightlylate) has given [a very insightful talk on mobile web performance](//youtu.be/4bZvq3nodf4?list=PLNYkxOF6rcIBTs2KPy1E6tIYaWoFcG3uj) at Chrome Dev Summit 2016, talking about how hard can we build performant web applications on mobile devices. Highly recommended.
...
<iframe width="700" height="525" src="//www.youtube.com/embed/ZLc8jysMqaw?ecver=1" frameborder="0" allowfullscreen></iframe>
...
What PWA is trying to solve are some fundamental problems of current web application model such as its hard dependencies to network and browser UIs. That' why PWA can be always beneficial no matter what architecture or what framework you actually used. [Addy Osmani](//medium.com/@addyosmani) would give a talk [Production Progressive Web Apps With JavaScript Frameworks](//events.google.com/io/schedule/?section=may-19&sid=e8436b55-ea89-4243-a644-5ecb319d9ef0) at this year's I/O (and [I/O 16](//youtu.be/srdKq0DckXQ?list=PLNYkxOF6rcIDz1TzmmMRBC-kd8zPRTQIP)). You won’t want to miss it!

---

Finally, I’d love to thank:

- my colleagues [YiSi Wang](//github.com/YiSiWang), [GuangHui Ren](//github.com/rguanghui), [JiyinYiyong](//medium.com/@jiyinyiyong) from Eleme
- collaborator [Michael Yeung](//medium.com/@micyeung), [Liam Spradlin](//medium.com/@LiamSpradlin) and other collaborators from Google
- collaborators from UC/Tencent

And special thanks to 

- invited reviewer, [Evan You](//medium.com/@youyuxi).
- Chrome “StackOverflow”, [Jake Archibald](//twitter.com/jaffathecake).
